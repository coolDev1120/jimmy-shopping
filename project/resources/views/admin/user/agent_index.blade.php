...
@section('content')  
					...
					<div class="content-area">
						<div class="mr-breadcrumb">
							<div class="row">
								<div class="col-lg-12">
										<h4 class="heading">{{ __("My Agent Customers") }}</h4>
										<ul class="links">
											<li>
												<a href="{{ route('admin.dashboard') }}">{{ __("Dashboard") }} </a>
											</li>
											<li>
												<a href="{{ route('admin-agent-user-index') }}">{{ __("My Agent Customers") }}</a>
											</li>
										</ul>
								</div>
							</div>
...
{{-- DATA TABLE --}}
    ...
    <script type="text/javascript">
		...
		var table = $('#geniustable').DataTable({
               ...
               processing: true,
               serverSide: true,
               ajax: "{{ route('admin-agent-user-datatables', Auth::user()->id) }}",
               columns: [
                        { data: 'name', name: 'name' },
