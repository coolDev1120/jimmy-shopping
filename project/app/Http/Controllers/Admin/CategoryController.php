...
class CategoryController extends Controller
{
    ...
    public function datatables()
    {
         ...
         return Datatables::of($datas)
                            ->addColumn('status', function(Category $data) {
                                ...
                                return '<div class="action-list"><select class="process select droplinks '.$class.'"><option data-val="1" value="'. route('admin-cat-status',['id1' => $data->id, 'id2' => 1]).'" '.$s.'>US Market</option><option data-val="0" value="'. route('admin-cat-status',['id1' => $data->id, 'id2' => 0]).'" '.$ns.'>Vietnam Market</option>/select></div>';
                            })
    ...
    public function addCategories(Request $request)
    {
        $categories = [
            ...
            "Kids’ Arts & Crafts",
            "Kids’ Kitchens",
            "Kids’ Outdoor Toys & Play",
            "Ride-on Toys & Wagons",
            "Toy Cars & Trucks",
            "Train Sets & Activity Tables"
        ];

        foreach($categories as $category)
        {
            $new_category = new Subcategory();
            $new_category->category_id = 23;
